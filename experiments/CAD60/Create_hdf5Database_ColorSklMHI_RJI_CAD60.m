% load CAD60_LOSOCV_ColorSklMHI_RJI_Data.mat

%% WRITING TO HDF5
% for Color Skl-MHI data
name='CAD60_ColorSklMHI_T15_P1';
% for RJI data
% name='CAD60_RJI_1_T15_P1';
filename=[ name '.h5'];
% filename=[name '.h5'];

chunksz=10;
all_datas  = ColorSklMHI_P1;
all_labels = all_labels_P1;
num_total_samples= floor(length(all_labels)/chunksz)*chunksz;

kk = randperm(num_total_samples);
train_img1 = double(all_datas(kk,:));
train_lbl1 = all_labels(kk,:);

data_=reshape(train_img1',62,62,3,num_total_samples);
data_disk = data_;
...
data_rd=h5read(filename, '/data', [1 1 1 10], [62, 62, 3, 10]);
